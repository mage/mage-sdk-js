...
Mage.prototype.init = function (cb) {
	if (!this.endpoint) {
		throw new Error('Please call Mage.setEndpoint before calling Mage.init');
	}
	...
	var url = this.endpoint + '/config?appName=' + this.appName;
	var that = this;

	hr.send('GET', url, null, null, null, function (err, res) {
		if (err) {
			return cb(err);
		}
...
Mage.prototype.configure = function (config) {
	...
	this.clientHostBaseUrl = this.endpoint;
	...
	// Update msgStreams urls with endpoint
	if (server.msgStream && server.msgStream.transports) {
		var transportsNames = Object.keys(server.msgStream.transports);
		for (var i = 0; i < transportsNames.length; i += 1) {
			var transportName = transportsNames[i];
			var transport = server.msgStream.transports[transportName];
			transport.url = this.endpoint + transport.url;
		}
	}

	// Update commandCenter url
	server.commandCenter.url = this.endpoint + server.commandCenter.url;
...
Mage.prototype.addModule = function (name, mod) {
	if (!this.config) {
		throw new Error('Please call Mage.init before calling any method');
	} else if (!this.canAddModule(name)) {
		return;
	}
